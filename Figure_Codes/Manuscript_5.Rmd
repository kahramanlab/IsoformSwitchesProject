...
mel_exp_matrix <- readRDS('/Users/admin/Desktop/TuPro_Paper/Codes/Codes_Figures/Data/NonRedundantTPMs/Skin_Melanoma/tuPro_tpm_NR.RDS')
...
[1] 56 common samples in RNA and DIA samples
...
ClassI_ENSP_Seq_Run_enst$Cohort <- 'Melanoma'

ClassII_ENSP_Seq_Run_enst$Class <- 'ClassII'
ClassII_ENSP_Seq_Run_enst$Cohort <- 'Melanoma'
...
write.csv(ClassI_II_Detected_Sequence_uniq, '/Users/admin/Desktop/TuPro_Paper/BJC/Revision/ClassI_II_Detected_Sequence_uniq_Melanoma.csv', row.names = FALSE) #sample specific
```
...
dMDT_Melanoma_sing_1_protein <- unique(ensg_enst_ensp_apris_v104 %>% dplyr::filter(Transcript.stable.ID %in% dMDT_Melanoma_sing_1) %>% dplyr::select(Protein.stable.ID) %>% dplyr::pull(Protein.stable.ID))

final_data %>%  dplyr::filter(Protein.Ids %in% dMDT_Melanoma_sing_1_protein) %>% dplyr::select(Protein.Ids) %>% dplyr::distinct() ## 1 cMDT belongs to Class I - and seen at proteomics level - sample specific

final_data_allcMDTs %>%  dplyr::filter(Protein.Ids %in% dMDT_Melanoma_sing_1_protein) %>% dplyr::select(Protein.Ids) %>% dplyr::distinct() ## 6 cMDT belongs to Class I - and seen at proteomics level - sample specific - total gives 7
```

length(unique(dMDT_Melanoma_sing_1))
[1] 1214
