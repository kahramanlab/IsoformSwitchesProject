...
```{r read_output_diann}
Main_output <- read_delim('/Users/admin/Desktop/TuPro_Paper/Codes/Codes_Figures/Data/Proteomics/AML/Main_Report.tsv', delim = '\t', col_names = TRUE)
head(Main_output)
...
dMDT_proteomics_dt_unique_peptide <- dMDT_proteomics_dt[dMDT_proteomics_dt$Protein.Ids %in% Switches_AML_ensps$Protein.stable.ID, ] 

MDT_proteomics_dt_unique_peptide <- MDT_proteomics_dt[MDT_proteomics_dt$Protein.Ids %in% Switches_AML_ensps_mdts$Protein.stable.ID, ] 
...
aml_exp_matrix <- readRDS('/Users/admin/Desktop/TuPro_Paper/Codes/Codes_Figures/Data/NonRedundantTPMs/Blood_AML/tuPro_tpm_NR.RDS')
...
ClassI_ENSP_Seq_Run_enst$Cohort <- 'AML'

ClassII_ENSP_Seq_Run_enst$Class <- 'ClassII'
ClassII_ENSP_Seq_Run_enst$Cohort <- 'AML'
...
write.csv(ClassI_II_Detected_Sequence_uniq, '/Users/admin/Desktop/TuPro_Paper/BJC/Revision/ClassI_II_Detected_Sequence_uniq_AML.csv', row.names = FALSE) #sample specific
```
...
dMDT_AML_sing_1_protein <- unique(ensg_enst_ensp_apris_v104 %>% dplyr::filter(Transcript.stable.ID %in% dMDT_AML_sing_1) %>% dplyr::select(Protein.stable.ID) %>% dplyr::pull(Protein.stable.ID))

final_data %>%  dplyr::filter(Protein.Ids %in% dMDT_AML_sing_1_protein) %>% dplyr::select(Protein.Ids) %>% dplyr::distinct() ## 1 cMDT belongs to Class I - and seen at proteomics level - sample specific

final_data_allcMDTs %>%  dplyr::filter(Protein.Ids %in% dMDT_AML_sing_1_protein) %>% dplyr::select(Protein.Ids) %>% dplyr::distinct() ## 2 cMDT belongs to Class I - and seen at proteomics level - sample specific - total gives 3
```
dMDT_AML_sing_1 - 338 cMDT
